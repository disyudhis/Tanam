...
class SearchFragment : Fragment() {
    ...
    private fun updateUI(filtered: TanamEntity?) {
        if (filtered == null) {
            showError(binding.tanamanHint, "Maaf data tidak ditemukan")
            binding.details.isVisible = false
            binding.judulTanaman.isVisible = false
            binding.deskripsiTanaman.isVisible = false
            binding.cara.isVisible = false
            binding.namaLatin.isVisible = false
            return
        } else {
